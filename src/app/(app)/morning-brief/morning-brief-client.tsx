...
import { dailyBrief, upcomingMoments, kpis as mockKpis } from "@/lib/data";
...
export function MorningBriefClient({
  ...
  const topMoves = loops.slice(0, 5);
  const [statuses, setStatuses] = React.useState<Record<string, "open" | "done" | "snoozed">>(
    Object.fromEntries(topMoves.map((l) => [l.id, l.status]))
  );
  ...
  const cashStuck = overdueInvoices.reduce((s, i) => s + i.amount, 0);
  const customersNeedingReply = customers.filter((c) => c.needsReply).length;
  ...
  const setStatus = (id: string, status: "open" | "done" | "snoozed", label: string) => {
    ...
    if (isLive) {
      setOpenLoopStatus(id, status).catch(() => toast.error("Couldn't save that — try again."));
    }
  };
  ...
  return (
    <div>
      <PageHeader
        eyebrow={dateLabel}
        title={`Good morning, ${founderFirstName}.`}
      ...
      <div className="mb-6 grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
        <MetricCard
          ...
          changePct={mockKpis.mtdRevenueChangePct}
        ...
        <MetricCard
          label="Cash stuck"
          value={`$${(cashStuck / 1000).toFixed(1)}k`}
          changeLabel={`${overdueInvoices.length} overdue invoices`}
          icon={WalletCards}
          tone="danger"
        />
        <MetricCard label="Customers needing reply" value={customersNeedingReply.toString()} icon={Users} tone="warning" />
        <MetricCard label="Open loops" value={loops.length.toString()} icon={ListChecks} tone="info" />
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <SurfaceCard className="p-5 sm:p-6">
          ...
          <div className="mt-3">
            <ProgressStrip
              total={loops.length}
              urgent={loops.filter((l) => l.priority === "urgent").length}
              revenueTied={loops.filter((l) => l.revenueTied).length}
            />
          </div>
          <div className="mt-4 space-y-2.5">
            {loops.slice(5, 9).map((loop) => (
